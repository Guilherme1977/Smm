...
if ($settings["sms_provider"] == "bizimsms"):
    ...
    function SMSUser($tel, $text) {
        ...
        $xml = "" . "<sms>" . "<username>" . $settings["sms_user"] . "</username>" . "<password>" . $settings["sms_pass"] . "</password>" . "<header>" . $settings["sms_title"] . "</header>" . "<validity>2880</validity>" . "<message>" . "<gsm>" . "<no>" . $tel . "</no>" . "</gsm>" . "<msg><![CDATA[" . $text . "]]></msg>" . "</message>" . "</sms>";
    ...
    function SMSToplu($tel, $text) {
        global $conn, $settings;
        $xml = "" . "<sms>" . "<username>" . $settings["sms_user"] . "</username>" . "<password>" . $settings["sms_pass"] . "</password>" . "<header>" . $settings["sms_title"] . "</header>" . "<validity>2880</validity>" . "<message>" . "<gsm>" . $tel . "</gsm>" . "<msg><![CDATA[" . $text . "]]></msg>" . "</message>" . "</sms>";
        $send = XMLPOST('http://sms.bizimsms.mobi:8080/api/smspost/v1', $xml);
        $sonuc = substr($send, 0, 2);
        if ($sonuc["0"] == 00) {
            return true;
