    ...
    foreach ($apiServices as $apiService):
        if ($service["api_service"] == $apiService["service"]):
            ...
            if ($apiService["rate"] != $extras["rate"]):
                ...
                $insert->execute(array("service" => $service["service_id"], "alert" => "Price of service #" . $service["service_id"] . " has been changed.", "date" => date("Y-m-d H:i:s"), "extra" => json_encode($extra)));
                if ($insert):
                    $there_change = $there_change + 1;
                endif;
            endif;
            if ($apiService["min"] != $extras["min"]):
                $extra = ["old" => $extras["min"], "new" => $apiService["min"]];
                $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
                $insert->execute(array("service" => $service["service_id"], "alert" => "Minimum quantity of service #" . $service["service_id"] . " has been changed.", "date" => date("Y-m-d H:i:s"), "extra" => json_encode($extra)));
                if ($insert):
                    $there_change = $there_change + 1;
                endif;
            endif;
            if ($apiService["max"] != $extras["max"]):
                ...
                $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
                $insert->execute(array("service" => $service["service_id"], "alert" => "Maximum quantity of service #" . $service["service_id"] . " has been changed.", "date" => date("Y-m-d H:i:s"), "extra" => json_encode($extra)));
                if ($insert):
                    $there_change = $there_change + 1;
                endif;
            endif;
            if ($service["api_servicetype"] == 1 && $there[$service["service_id"]]):
                ...
                $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
                $insert->execute(array("service" => $service["service_id"], "alert" => "Service #" . $service["service_id"] . " has been reactivated by the provider.", "date" => date("Y-m-d H:i:s"), "extra" => json_encode($extra)));
                if ($insert):
                    $there_change = $there_change + 1;
                endif;
            else:
...
foreach ($there as $service => $type):
    ...
    if ($type == 0 && $serviceDetail["api_servicetype"] == 2):
        $extra = ["old" => "Active at Provider", "new" => "Inactive at Provider"];
        ...
        $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
        $insert->execute(array("service" => $service, "alert" => "#" . $service . " has been deleted by the provider.", "date" => date("Y-m-d H:i:s"), "extra" => json_encode($extra)));
        if ($update):
            $there_change = $there_change + 1;
        endif;
    endif;
...
if ($settings["alert_serviceapialert"] == 2 && $there_change):
    ...
    if ($sendmail):
        sendMail(["subject" => "Information changed by provider", "body" => "You have services whose information is changed by the service provider..", "mail" => $settings["admin_mail"]]);
    endif;
endif;
