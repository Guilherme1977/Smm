                    ...
                    if (round(($now - $last_check) / 60) < $order["dripfeed_interval"]):
                        ...
                        elseif ($order["dripfeed_delivery"] >= $order["dripfeed_runs"]):
                                ...
                                if ($order["api_type"] == 1):
                                    ...
                                    $insert = $insert->execute(array("c_id" => $order["client_id"], "detail" => json_encode($getOrder), "error" => $error, "s_id" => $order["service_id"], "quantity" => $quantity, "price" => $order["dripfeed_totalcharges"] / $order["dripfeed_runs"], "url" => $link, "create" => date("Y.m.d H:i:s"), "extra" => $extras, "order_id" => $order_id, "last_check" => date("Y.m.d H:i:s"), "api" => $order["api_id"], "api_serviceid" => $order["api_service"], "dripfeed_id" => $order["order_id"], "profit" => $api_charge * $currencycharge, "api_charge" => $api_charge, "api_currencycharge" => $currencycharge));
                                    if ($insert):
