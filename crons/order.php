...
foreach ($orders as $order):
    ...
    if ($order["api_type"] == 1):
        ...
        elseif ($order["api_type"] == 3):
            $orderstatus = $smmapi->action(array('api_token' => $order["api_key"], 'status' => 'balance', 'order' => $order["api_orderid"]), $order["api_url"]);
            $balance = $smmapi->action(array('api_token' => $order["api_key"], 'action' => 'balance'), $order["api_url"]);
            $api_charge = $orderstatus->charge;
            $currency = $balance->currency;
            $statu = str_replace(" ", "", strtolower($orderstatus->status));
            $start = $orderstatus->start_counter;
            $remains = $orderstatus->remains;
        else:
        endif;
        ...
        if ($statu == "canceled" || $statu == "cancel" || $statu == "refunded"):
            ...
            $report = $report->execute(array("date" => date("Y-m-d H:i:s"), "id" => $order["client_id"], "action" => "Order #" . $orderid . " was canceled and " . $order["order_charge"] . " " . $settings["currency"] . " balance has been refund. Old balance:" . $clientBalance . " / New balance:" . $user["balance"], "ip" => "127.0.0.1"));
        ...
        elseif ($statu == 'complete' || $statu == 'completed'):
            $conn->beginTransaction();
            $report = $conn->prepare("INSERT INTO client_report SET client_id=:id, action=:action, report_ip=:ip, report_date=:date ");
            $report = $report->execute(array("date" => date("Y-m-d H:i:s"), "id" => $order["client_id"], "action" => "Order #" . $orderid . " completed.", "ip" => "127.0.0.1"));
        ...
        elseif ($statu == "partial"):
            ...
            $user = $user->fetch(PDO::FETCH_ASSOC);
            $report = $conn->prepare("INSERT INTO client_report SET client_id=:id, action=:action, report_ip=:ip, report_date=:date ");
            $report = $report->execute(array("date" => date("Y-m-d H:i:s"), "id" => $order["client_id"], "action" => "Order #" . $orderid . " completed as partial and " . $return_price . " " . $settings["currency"] . " balance has been refund. Old balance:" . $clientBalance . " / New balance:" . $user["balance"], "ip" => "127.0.0.1"));
            $update = $conn->prepare("UPDATE orders SET order_detail=:detail, order_start=:start, order_finish=:finish, order_remains=:remains, order_status=:status, order_charge=:charge, api_charge=:api_charge, order_profit=:order_profit WHERE order_id=:id ");
