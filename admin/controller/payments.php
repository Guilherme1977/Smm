...
if ($_POST):
    ...
    elseif (route(2) == "new-bank"):
        ...
        elseif (!countRow(["table" => "clients", "where" => ["username" => $username]])):
            ...
            $insert = $insert->execute(array("status" => 3, "delivery" => 2, "bank" => $bank, "mode" => "Manual", "amount" => $amount, "method" => 6, "note" => $note, "date" => date("Y-m-d H:i:s"), "date2" => date("Y-m-d H:i:s"), "balance" => $user["balance"], "client_id" => $user["client_id"]));
            $update2 = $conn->prepare("UPDATE clients SET balance=:balance WHERE client_id=:id ");
    ...
    elseif (route(2) == "new-online"):
        ...
        elseif (!countRow(["table" => "clients", "where" => ["username" => $username]])):
            ...
            $insert = $insert->execute(array("status" => 3, "delivery" => 2, "mode" => "Manual", "amount" => $amount, "method" => $method, "note" => $note, "date" => date("Y-m-d H:i:s"), "date2" => date("Y-m-d H:i:s"), "balance" => $user["balance"], "client_id" => $user["client_id"]));
            $update2 = $conn->prepare("UPDATE clients SET balance=:balance WHERE client_id=:id ");
            $update2 = $update2->execute(array("id" => $user["client_id"], "balance" => $amount + $user["balance"]));
            if ($update2 && $insert):
                $conn->commit();
