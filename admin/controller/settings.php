...
$menuList = ["General settings" => "general", "Page settings" => "pages", "Payment methods" => "payment-methods", "Bank accounts" => "bank-accounts", "Payment bonuses" => "payment-bonuses", "Alert settings" => "alert", "Service providers" => "providers"];
        ...
        if(UMH != true):
            if ($_POST):
                ...
                elseif (empty($currency) or empty($csymbol)):
                    ...
                    $update = $conn->prepare("UPDATE settings SET whatsapp=:whatsapp, mail=:mail, site_maintenance=:site_maintenance, resetpass_page=:resetpass_page, resetpass_sms=:resetpass_sms, resetpass_email=:resetpass_email, site_name=:name, site_logo=:logo, favicon=:fv, site_title=:title, site_keywords=:keys, site_description=:desc, recaptcha=:recaptcha, recaptcha_key=:recaptcha_key, recaptcha_secret=:recaptcha_secret, ticket_system=:ticket_system, register_page=:registration_page, service_list=:service_list, service_speed=:service_speed, custom_header=:custom_header, custom_footer=:custom_footer, currency=:currency, csymbol=:csymbol WHERE id=:id ");
                    $update->execute(array("id" => 1, "resetpass_page" => $resetpass, "site_maintenance" => $site_maintenance, "mail" => $mail, "whatsapp" => $whatsapp, "resetpass_sms" => $resetsms, "resetpass_email" => $resetmail, "name" => $name, "logo" => $logo_newname, "fv" => $fv_newname, "title" => $title, "keys" => $keywords, "desc" => $description, "recaptcha" => $recaptcha, "recaptcha_secret" => $recaptcha_secret, "recaptcha_key" => $recaptcha_key, "ticket_system" => $ticket_system, "registration_page" => $registration_page, "service_list" => $service_list, "service_speed" => $service_speed, "custom_footer" => $custom_footer, "custom_header" => $custom_header, "currency" => $currency, "csymbol" => $csymbol));
                    if ($update):
    ...
    if ($access):
        ...
        elseif (route(3) == "delete"):
            if(UMH != true):
                ...
                $delete->execute(array("id" => $provider_id));
                if ($delete):
                    $_SESSION["client"]["data"]["success"] = 1;
                    $_SESSION["client"]["data"]["successText"] = "Successful";
                else:
